...
function setup() {
	...
	/*for (var index = 0; index <coins.length; index++){
		if(Matter.SAT.collides(knight.body, coins[index -1])){}
		world.remove(world, coins[index -1])
	}*/
 	for(var index =0;index < coins.length; index++){
	if(knight.body.position.x - coins[index].body.position.x <50 && coins[index].body.position.x - knight.body.position.x <50 ){
		world.remove(world, coins[index].body)
		coins[index].body = null
	}
}
...
function keyPressed(){
	...
	if(knight.body.position.x - enemy1.body.position.x <50 && enemy1.body.position.x - knight.body.position.x <50 && 
		knight.body.position.y - enemy1.body.position.y <20 && enemy1.body.position.y - knight.body.position.y <20 && keyCode ===32){
		enemy1.destroy();
	}
	if(knight.body.position.x - enemy2.body.position.x <70 && enemy2.body.position.x - knight.body.position.x <70 && 
		knight.body.position.y - enemy2.body.position.y <20 && enemy2.body.position.y - knight.body.position.y <20 && keyCode ===32){
		enemy2.destroy();
	}
